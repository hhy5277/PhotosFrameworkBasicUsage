...
- (id)initWithImages:(NSArray<UIImage *> *)images originImageViews:(NSArray<UIImageView *> *)originImageViews selectPage:(NSInteger)selectPage {
    if (self = [super init]) {
        self.modalPresentationStyle = UIModalPresentationOverCurrentContext;
        self.transitioningDelegate = self;
        self.images = images;
...
- (LZImageBrowserMainView *)browserMainView {
    if (_browserMainView == nil) {
        if ([self.imageUrls count] > 0) {
            ...
            _browserMainView = [LZImageBrowserMainView imageBrowserMainViewImages:self.images originImageViews:self.originImageViews selectPage:self.selectPage];
        }
